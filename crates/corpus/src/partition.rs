...
/// The single source of truth for the `node_id_partitions` table's
/// schema. Its DDL is rendered from this spec.
pub(crate) const SPEC: TableSpec = TableSpec {
    name: "node_id_partitions",
    comment: None,
    columns: &[
        ColumnSpec::int("partition_idx").primary_key(),
        ColumnSpec::int("book_root_id").not_null().unique(),
        ColumnSpec::int("intake_id").not_null().unique(),
        ColumnSpec::int("next_local_id").not_null().default("2"),
        ColumnSpec::text("allocated_at").not_null(),
    ],
    composite_pk: None,
    uniques: &[],
    table_checks: &[],
...
/// Existence probe run before allocating: does this intake already own a
/// partition?
const SELECT_IDX_BY_INTAKE: &str =
    "SELECT partition_idx FROM node_id_partitions WHERE intake_id = :intake_id";

/// Insert a fresh partition row. The cursor starts at offset 2, one past
/// the reserved root; the timestamp is generated by SQLite so the whole
/// crate shares a single timestamp source.
const INSERT_PARTITION_SQL: &str = "INSERT INTO node_id_partitions \
     (partition_idx, book_root_id, intake_id, next_local_id, allocated_at) \
     VALUES (:partition_idx, :book_root_id, :intake_id, 2, \
             strftime('%Y-%m-%dT%H:%M:%SZ', 'now')) \
     RETURNING allocated_at";

/// Read a partition's id cursor.
const SELECT_NEXT_LOCAL_ID: &str =
    "SELECT next_local_id FROM node_id_partitions WHERE partition_idx = :partition_idx";

/// Advance a partition's id cursor.
const UPDATE_NEXT_LOCAL_ID: &str =
    "UPDATE node_id_partitions SET next_local_id = :next WHERE partition_idx = :partition_idx";

/// Delete every node of a book. A book's nodes all carry its root id in
/// `book_root_id`, so one indexed delete clears the whole tree. This
/// statement targets the `nodes` table, which the node module owns.
const DELETE_BOOK_NODES: &str = "DELETE FROM nodes WHERE book_root_id = :book_root_id";

/// Delete a partition's allocator row.
const DELETE_PARTITION_SQL: &str =
    "DELETE FROM node_id_partitions WHERE partition_idx = :partition_idx";
...
impl Partition {
    /// Build a [`Partition`] from a `node_id_partitions` row. Columns are
    /// read by name, so the row's column order is irrelevant.
    fn from_row(row: &Row<'_>) -> rusqlite::Result<Partition> {
        Ok(Partition {
            idx: PartitionIdx::new(row.get("partition_idx")?),
            book_root_id: NodeId::new(row.get("book_root_id")?),
            intake_id: row.get("intake_id")?,
            next_local_id: row.get("next_local_id")?,
...
impl Corpus {
    ...
    pub fn partition_for_intake(&self, intake_id: i64) -> Result<Option<Partition>> {
        let partition = self
            ...
            .query_row(
                &format!(
                    "SELECT {} FROM node_id_partitions WHERE intake_id = :intake_id",
                    SPEC.select_list()
                ),
                named_params! { ":intake_id": intake_id },
                Partition::from_row,
    ...
    pub fn allocate_node_ids(
        ...
        let next: i64 = tx
            .query_row(
                SELECT_NEXT_LOCAL_ID,
                named_params! { ":partition_idx": partition.get() },
                |row| row.get(0),
            )
            .optional()?
        ...
        tx.execute(
            UPDATE_NEXT_LOCAL_ID,
            named_params! { ":next": last + 1, ":partition_idx": partition.get() },
        )?;
        tx.commit()?;
    ...
    pub fn drop_partition(&mut self, partition: PartitionIdx) -> Result<()> {
        ...
        tx.execute(
            DELETE_BOOK_NODES,
            named_params! { ":book_root_id": partition.root().get() },
        )?;
        tx.execute(
...
mod tests {
    ...
    #[test]
    fn a_partition_round_trips_every_field() {
        let mut corpus = Corpus::open_in_memory().expect("open");
        let allocated = corpus.allocate_partition(12).expect("allocate");

        // Fetch through `from_row` and confirm every column survives.
        let fetched = corpus
            .partition_for_intake(12)
            .expect("lookup")
            .expect("present");
        assert_eq!(fetched.idx, allocated.idx);
        assert_eq!(fetched.book_root_id, allocated.book_root_id);
        assert_eq!(fetched.intake_id, 12);
